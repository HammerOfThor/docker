...
Options:
  ...
  -q, --quiet           Only show numeric IDs
      --sort-by string  Customize list order, sort by field[:order]. e.g. "ID:asc,CreatedAt:desc"
```
...
should be either `asc`(ascendant) or `desc`(descendant). Meanings of each `field`
value has been described in [`Formatting`](#formatting) section.
