...
func (s *DockerSuite) TestApiStatsAllNoStreamGetCpu(c *check.C) {
	...
	resp, body, err := sockRequestRaw("GET", "/containers/-/stats?stream=false", nil, "")
	c.Assert(err, checker.IsNil)
	c.Assert(resp.StatusCode, checker.Equals, http.StatusOK)
	c.Assert(resp.Header.Get("Content-Type"), checker.Equals, "application/json")
...
test_stats_all:
	...
	_, body, err = sockRequestRaw("GET", "/containers/-/stats", nil, "")
	c.Assert(err, checker.IsNil)
	body.Close()
...
func getNetworkStatsForAll(c *check.C, id string) map[string]types.NetworkStats {
	...
	_, body, err := sockRequestRaw("GET", "/containers/-/stats?stream=false", nil, "")
	c.Assert(err, checker.IsNil)
